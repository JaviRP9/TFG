...
table_header = ["UID", "X (m/s²)", "Y (m/s²)", "Z (m/s²)", "Packets received", "Packets lost", "Error rate (%)", "Bytes received", "Bitrate (Bps)", "Var X", "Var Y", "Var Z"]
stats_dic = {}
...
def print_stats_table():
    ...
    while 1:
        ...
        for key in stats_dic:
            myTable.add_row([key, stats_dic[key][8], stats_dic[key][9], stats_dic[key][10], stats_dic[key][2], stats_dic[key][3], round(stats_dic[key][4], 2), stats_dic[key][5], round(stats_dic[key][6],1), stats_dic[key][11],stats_dic[key][12],stats_dic[key][13]])
...
while (True):
    ...
    if euid in stats_dic:
        if stats_dic[euid][0] != session_rand:
            stats_dic[euid] = [session_rand, ctr, 1, 0, 0, len(payload), 0, time.time(), x, y, z, 0, 0, 0]
        else:
            ...
            y_vec.append(y)
            z_vec.append(z)
            stats_dic[euid][11] = np.var(x_vec)
            stats_dic[euid][12] = np.var(y_vec)
            stats_dic[euid][13] = np.var(z_vec)
    else:
        stats_dic[euid] = [session_rand, ctr, 1, 0, 0, len(payload), 0, time.time(), x, y, z , 0, 0, 0]